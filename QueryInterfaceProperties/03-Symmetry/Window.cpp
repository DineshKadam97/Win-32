...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (iMsg)
	{
		case WM_CREATE: //CLSCTX_INPROC_SERVER  => com server kontya prakaracha aahe he sangto, class context in process server(jevha dll asto aani aplaych machine var asto tevha as lihitat)
			...
			//Proving Symmetry Property of QueryInterface
			hr = pISubtract->QueryInterface(IID_ISUM, (void**)&pISum_Again);
			if (FAILED(hr)) //only one CoCreateInstance per DLL
			{
				MessageBox(NULL, TEXT("QueryInterface FAILED TO OBTAIN pISum_Again INTERFACE!!"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
				DestroyWindow(hwnd);
			}

			if (pISum == pISum_Again)
			{
				MessageBox(NULL, TEXT("SYMMETRY PROVED!!"), TEXT("COM"), MB_OK);
			}
			DestroyWindow(hwnd);
