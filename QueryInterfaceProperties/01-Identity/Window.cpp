...
IUnknown* pIUnknown_FromIsum = NULL;
IUnknown* pIUnknown_FromSubtract = NULL;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (iMsg)
	{
		case WM_CREATE: //CLSCTX_INPROC_SERVER  => com server kontya prakaracha aahe he sangto, class context in process server(jevha dll asto aani aplaych machine var asto tevha as lihitat)
			...
			//Provinf Identity Property of QueryInterface
			hr = pISum->QueryInterface(IID_IUnknown, (void **)&pIUnknown_FromIsum);

			if (FAILED(hr)) //only one CoCreateInstance per DLL
			{
				MessageBox(NULL, TEXT("QueryInterface FAILED TO OBTAIN pIUnknown_FromIsum INTERFACE!!"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
				DestroyWindow(hwnd);
			}

			hr = pISubtract->QueryInterface(IID_IUnknown, (void**)&pIUnknown_FromSubtract);

			if (FAILED(hr)) //only one CoCreateInstance per DLL
			{
				MessageBox(NULL, TEXT("QueryInterface FAILED TO OBTAIN pIUnknown_FromSubtract INTERFACE!!"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
				DestroyWindow(hwnd);
			}

			if (pIUnknown_FromSubtract == pIUnknown_FromIsum)
			{
				MessageBox(NULL, TEXT("IDENTITY PROPERTY OF QUERY INTERFACE IS PROVED!!"), TEXT("COM identity"),MB_OK);
			}
...
{
	//code
	if (pIUnknown_FromSubtract)
	{
		pIUnknown_FromSubtract->Release();
		pIUnknown_FromSubtract = NULL;
	}

	if (pIUnknown_FromIsum)
	{
		pIUnknown_FromIsum->Release();
		pIUnknown_FromIsum = NULL;
	}
	if (pISubtract)
	{
		pISubtract->Release();
