...
#include "Window.h"
#include "CombinedHeaderFileForAggregationClient.h"
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	int iSum = 0, iSub = 0, iMult = 0, iDiv = 0;
	TCHAR str[255];
	TCHAR str2[255];
	...
	{
		case WM_CREATE:  
			//Aggregation SubFlow 1 : In end client calling co-create instance to obtain outer's interface.
			...
			//if (FAILED(hr))
			//{
			//	MessageBox(NULL, TEXT("QueryInterface FAILED TO OBTAIN IMULTIPLICATION INTERFACE!!"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
			//	DestroyWindow(hwnd);
			//}

			//pIMultiplication->MultiplicationOfTwoIntegers(20, 11, &iMult);
			//wsprintf(str, TEXT("Multiplication is : %d"), iMult);
			//MessageBox(NULL, str, TEXT("MULTIPLICATION"), MB_OK);
			...
			//pIDivision->DivisionOfTwoIntegers(20, 10, &iDiv);
			//wsprintf(str, TEXT("Ddivision is : %d"), iDiv);
			//MessageBox(NULL, str, TEXT("DIVISION"), MB_OK);

			//Aggregation sub-flow 2 :  Calling QueryInterface on outer interface pointer to obtain another one of outer's interfaces
			hr = pISum->QueryInterface(IID_ISubtract, (void**)pISubtract);
			if (FAILED(hr)) //only one CoCreateInstance per DLL
			{
				MessageBox(NULL, TEXT("QueryInterface FAILED TO OBTAIN ISUBTRACT INTERFACE FROM pIsum!!"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
				DestroyWindow(hwnd);
			}
			...
			//Aggregation sub-flow 3 :  Calling QueryInterface on outer interface pointer to obtain another one of inner's interfaces
			...
			// Aggregation sub - flow 4 :  Calling QueryInterface on inner interface pointer to obtain another one of inner's interfaces
			hr = pIMultiplication->QueryInterface(IID_IDIVISION, (void**)pIDivision);
			if (FAILED(hr)) 
			{
				MessageBox(NULL, TEXT("QueryInterface FAILED TO OBTAIN IDIVISION INTERFACE!!"), TEXT("COM ERROR"), MB_OK | MB_ICONERROR);
				DestroyWindow(hwnd);
			}
